    ...
    Private Sub dashboardTS_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            ElseIf Request.Params("mode") = "new" Or Request.Params("mode") = "Renewal" Or Request.Params("mode") = "Cancellation" Or Request.Params("mode") = "Change" Then
                ...
                ddlNewWorkArea.Items.Add(New ListItem("Ash Pond Civil", "Ash Pond Civil"))
ddlNewWorkArea.Items.Add(New ListItem("AHP(M)", "AHP(M)"))
		...
		ddlNewWorkArea.Items.Add(New ListItem("Mechanical RODM", "Mechanical RODM"))
                ...
                ddlNewWorkArea.Items.Add(New ListItem("Mechanical PT", "Mechanical PT"))
                ...
                ddlNewWorkArea.Items.Add(New ListItem("Structural Works", "Structural Works"))
                ...
                If Not Request.Params("id") Is Nothing Then
                    ...
                    If Not Request.Params("approvetype") Is Nothing Then
                        ...
                        ' Dim apchangeprover2 = getDBsingle("select level from agency where email = '" & Session("email") & "' and level='approver2' limit 1", "gpdb")
                    ...
                    ElseIf Request.Params("mode").ToString.Contains("Change") Then
                        ...
                        txtNewValidityStart.Enabled = True
                        txtNewValidityEnd.Enabled = True
            ...
            ElseIf Request.Params("mode") = "renew" Then
                ...
                loadRenewalGrid()
            ElseIf Request.Params("mode") = "modifytemp" Then
             'lbModTempApprover.Text = "BC " &  newApprover
                If newApprover.Contains("Error") Then  ''Pass url or onlybifpcl for all bifplc executives
                    lbError.Text = "mt Your email " & Session("email") & " don't have Authorisation to Access.<br/> Only for BIFPCL Executives."
                    Exit Sub
                End If
                'ddlUidNew.Items.Insert(1, "Common Place")
                pnlModifyTemp.Visible = True
                ddlModTempAgency.DataSource = getDBTable("select distinct agency   from agency order by agency asc", "gpdb")
                ddlModTempAgency.DataBind()
                If Not (newApprover.Contains("Approver EPC") Or newApprover.Contains("BIFPCL")) Then
                    ddlModTempAgency.SelectedValue = newApprover
                    ddlModTempAgency.Enabled = False
                End If
                If Not Request.Params("agency") Is Nothing Then
                    ddlModTempAgency.SelectedValue = Server.UrlDecode(Request.Params("agency").ToString)
                End If
                ' Dim approver1 = getDBsingle("select level from agency where email = '" & Session("email") & "' and level='approver1' limit 1", "gpdb")
                ' If Not approver1.Contains("Error") Then lbRenewApprover.Text = "Approver EPC"
                ...
                ' Dim approver2 = getDBsingle("select level from agency where email = '" & Session("email") & "' and level='approver2' limit 1", "gpdb")
                ' If Not approver2.Contains("Error") Then lbRenewApprover.Text = "Approver BIFPCL"
               lbModTempApprover.Text = newApprover
                loadModTempGrid()
    ...
    Private Function loadDPR() As Boolean
        ...
        Try
            ...
            If rblReportType.SelectedValue = "ID" Then
                ...
                If rblReportStatus.SelectedValue = "ApprovedValid" Then
                    myStatus = "Approved"
                    qPart1 = " , strftime('%d.%m.%Y',approve2_dt) as 'Approve2 Date',  last_renewal_issue_dt as 'Last Renewal Date' "
                    dateFilter = dateFilter & " and (validity_end >= current_date) "
                End If

                q = "select uid,id " & agency & " ,  name , father , sex , age ,strftime('%d.%m.%Y',ifnull(dob,'1900-01-01')) as 'DoB' , 'Village: ' || ifnull(village,'') || ' PO:' || ifnull(po,'') || ' PS:' || ifnull(ps,'') || 'Dist:' || ifnull(district,'') as 'Address', nationality as 'Nationality', nid as 'NID/Passport',  desig, cat ,strftime('%d.%m.%Y', validity_start) as `Validity Start`,strftime('%d.%m.%Y', validity_end) as `Validity End`, visa, strftime('%d.%m.%Y', ifnull(visaexp,'1999-01-01')) as visaexp,  mainagency as 'Work Partner', subagency, compliance, status, area " & qPart1 & " ,corona, pre_test_date as `PreTest`, post_test_date as `PostTest`, ifnull(last_renewal_apply_dt,first_apply_dt) as 'Apply Date', cancelremark, remark from gatepass where status like '%" & myStatus & "%' and agency like  '%" & ddlReportAgency.SelectedValue & "%' " & dateFilter & " order by id desc"
            ...
            ElseIf rblReportType.SelectedValue = "PC" Then
                q = "select uid,id , agency ,  name , father , nid as 'NID/Passport',  desig, strftime('%d.%m.%Y', validity_start) as `Validity Start`,strftime('%d.%m.%Y', validity_end) as `Validity End`, status , corona, last_updated from gatepass where agency like  '%" & ddlReportAgency.SelectedValue & "%' and corona = 'PC' " & dateFilter & "  order by id desc"
            ...
            ElseIf rblReportType.SelectedValue = "agency" Then
                ...
                End If
                q = "select agency, status, count(id) as 'Total ID', nationality from gatepass where agency in (select agency from agency) " & dateFilter & "  group by agency, status order by agency"
                Dim cummDT = getDBTable(q, "gpdb")
                    ...
                    q = "select agency, count(id) as 'Total ID' from gatepass where agency in (select agency from agency) and compliance = 'Height' and status = 'Approved' group by agency order by agency"
                ...
                q = "select agency, count(id) as 'Total ID' from gatepass where agency in (select agency from agency) and status = 'Approved'  and validity_end >= current_date   group by agency order by agency"
                Dim validDT = getDBTable(q, "gpdb")
                 q = "select agency, count(id) as 'Total ID' from gatepass where agency in (select agency from agency) and status = 'Approved'  and nationality like 'Indian' group by agency order by agency"
                Dim indianDT = getDBTable(q, "gpdb")
                 q = "select agency, count(id) as 'Total ID' from gatepass where agency in (select agency from agency) and status = 'Approved'  and nationality like 'Bangladeshi' group by agency order by agency"
                Dim bdDT = getDBTable(q, "gpdb")
                ...
                finalDT1.Columns.Add("ValidPass")
                finalDT1.Columns.Add("TempPass")
                 finalDT1.Columns.Add("IN")
                  finalDT1.Columns.Add("BD")
                finalDT1.Columns.Add("RevApprovedPass")
                finalDT1.Columns.Add("RevValidPass")
                ...
                For Each nwRow In agDT.Rows
                    ...
                    Dim validRow = validDT.Select("agency = '" & nwRow("agency") & "' ")
                    Dim valid As Double = If(validRow.Length = 1, validRow(0)(1), 0)
                    tmpRow("ValidPass") = valid.ToString
                   Dim TempPassRow = cummDT.Select("agency = '" & nwRow("agency") & "' and status = 'Temp'")
                    Dim TempPass As Double = If(TempPassRow.Length = 1, TempPassRow(0)(2), 0)
                    tmpRow("TempPass") = TempPass.ToString
                    Dim HIGHLYSKILLED_INRow = indianDT.Select("agency = '" & nwRow("agency") & "' ")
                    Dim HIGHLYSKILLED_IN As Double = If(HIGHLYSKILLED_INRow.Length = 1, HIGHLYSKILLED_INRow(0)(1), 0)
                    tmpRow("IN") = HIGHLYSKILLED_IN.ToString
                    Dim UNSKILLED_BDRow = bdDT.Select("agency = '" & nwRow("agency") & "' ")
                    Dim UNSKILLED_BD As Double = If(UNSKILLED_BDRow.Length = 1, UNSKILLED_BDRow(0)(1), 0)
                    tmpRow("BD") = UNSKILLED_BD.ToString
                    ...
                    tmpRow("RevApprovedPass") = Int32.Parse(tmpRow("ApprovedPass")) + Int32.Parse(tmpRow("EPCPendingReNew")) + Int32.Parse(tmpRow("BIFPCLPendingReNew"))
                     tmpRow("RevValidPass") = Int32.Parse(tmpRow("ValidPass")) + Int32.Parse(tmpRow("EPCPendingReNew")) + Int32.Parse(tmpRow("BIFPCLPendingReNew"))
                ...
                gvReport.DataSource = finalDT1
                    ...
                    divReportAgency.InnerHtml = "Summary Reports"
                     divReportMessage.InnerHtml = rblReportType.SelectedItem.text
                    Return True
    ...
    Sub loadCoverGrid()
        ...
        End If
        If rblCoverStatus.SelectedValue = "Temp" Then status = " status = 'Temp' "
        ...
        If mydt.Rows.Count = 0 Then
            ...
            Exit Sub
        End If
        lbMessageCover.Text = mydt.Rows.Count & " records found for " & ddlCoverAgency.SelectedValue & " status " & status '& q
        gvCover.DataSource = mydt
        gvCover.DataBind()
    End Sub
    Sub reloadGPData(ByVal status As String)
        ...
        Try
            ...
            ElseIf lbNewApproverType.Text = "Cancellation" Then
                qPart1 = " ,last_renewal_apply_by as apply_by, strftime('%d.%m.%Y',last_renewal_apply_dt) as apply_dt "
            End If
            q = "select id,sn,name,father,sex,age,strftime('%d.%m.%Y',dob) as dob,nationality,nid,mainagency,agency,subagency,desig,cat,status,mainagency,village,po,ps,district,cell,bgroup,area, strftime('%d.%m.%Y',validity_start) as validity_start,strftime('%d.%m.%Y',validity_end) as validity_end, strftime('%d.%m.%Y',pre_test_date) as pre_test_date, strftime('%d.%m.%Y',post_test_date) as post_test_date, visa, strftime('%d.%m.%Y',visaexp) as visaexp, cancelremark, last_updated,lastupdateby, corona, compliance,temp " & qPart1 & " from gatepass where id = " & lbID.Text & " limit 1"
            ...
            txtNewPreTestDate.Text = If(IsDBNull(mydt.Rows(0)("pre_test_date")), "", mydt.Rows(0)("pre_test_date")).ToString.Replace("NA", "")
            txtNewPostTestDate.Text = If(IsDBNull(mydt.Rows(0)("post_test_date")), "", mydt.Rows(0)("post_test_date")).ToString.Replace("NA", "")
            ...
            txtNewTemperature.Text = If(IsDBNull(mydt.Rows(0)("temp")), "", mydt.Rows(0)("temp")).ToString.Replace("NA", "")
            ...
            Dim corona = If(IsDBNull(mydt.Rows(0)("corona")), "", mydt.Rows(0)("corona"))
            ...
            If String.IsNullOrEmpty(cancelremark) Then
                rblNewCancelRemark.SelectedValue = "Red"
            Else
                rblNewCancelRemark.SelectedValue = cancelremark
            End If
            If String.IsNullOrEmpty(corona) Then
                rblCorona.SelectedValue = "NA"
            Else
                rblCorona.SelectedValue = corona
            End If
    ...
    End Sub
    Sub loadModTempGrid()
        Dim q = "select uid,id, name, agency,desig,nid, strftime('%d.%m.%Y', validity_start) as validity_start,strftime('%d.%m.%Y', validity_end) as validity_end, status from gatepass where status = 'Temp' and agency like '" & ddlModTempAgency.SelectedValue & "' order by last_updated desc"
        Dim mydt = getDBTable(q, "gpdb")
        lbModTempMessage.Text = mydt.Rows.Count & " Records found " '& q
        gvModifyTemp.DataSource = mydt
        gvModifyTemp.DataBind()

    End Sub
    Sub loadCancelGrid()
        Dim q = "select uid,id, name, agency,desig,nid, strftime('%d.%m.%Y', validity_start) as validity_start,strftime('%d.%m.%Y', validity_end) as validity_end, status from gatepass where (status = 'Approved' or status = 'Temp') and agency like '" & ddlCancelAgency.SelectedValue & "' order by last_updated desc"
        Dim mydt = getDBTable(q, "gpdb")
    ...
    End Sub
    Sub loadModifyGrid()
        Dim q = "select uid,id, name, agency,desig,nid, strftime('%d.%m.%Y', validity_start) as validity_start,strftime('%d.%m.%Y', validity_end) as validity_end, status from gatepass where (status = 'Approved' or status = 'Temp')  and agency like '" & ddlMoifyAgency.SelectedValue & "' order by last_updated desc"
        Dim mydt = getDBTable(q, "gpdb")
    ...
    Private Sub btnSumbitPass_Click(sender As Object, e As EventArgs) Handles btnSumbitPass.Click
        ...
        If String.IsNullOrEmpty(txtNewValidityStart.Text) Or String.IsNullOrEmpty(txtNewValidityEnd.Text) Then
            ...
            End If
        End If
        If String.IsNullOrEmpty(txtNewPreTestDate.Text) Then
            lbBookMessage.Text = "Corona Pre Test Date can't be blank"
            lbBookMessageTop.Text = lbBookMessage.Text
            Exit Sub

        End If
        If String.IsNullOrEmpty(txtNewPostTestDate.Text) And rblNewPassType.SelectedValue = "P" Then
            lbBookMessage.Text = "Corona Post Test Date can't be blank"
            lbBookMessageTop.Text = lbBookMessage.Text
            Exit Sub
        ...
        if rblNewPassType.SelectedValue = "T" then txtNewPostTestDate.Text = txtNewPreTestDate.Text
        Dim pre_test_date = DateTime.ParseExact(txtNewPreTestDate.Text, "dd.M.yyyy", Nothing)
        Dim post_test_date = DateTime.ParseExact(txtNewPostTestDate.Text, "dd.M.yyyy", Nothing)
        Dim temperature As Double
        If Not String.IsNullOrEmpty(txtNewTemperature.Text) Then
            Try
                Double.TryParse(txtNewTemperature.Text.Trim, temperature)
            Catch exT As Exception
                lbBookMessage.Text = "Temperature is inValid. Eg. 94.6"
                lbBookMessageTop.Text = lbBookMessage.Text
                Exit Sub
            End Try
        End If
        ...
        If lbNewMode.Text = "New" Then
            ...
            If rblNewPassType.SelectedValue = "T" Then status = "Temp"
            q = "insert into gatepass (id,sn,name,father,sex,age,dob,nationality,nid,mainagency,agency,subagency,desig, cat,compliance,status,mainagency,village,po,ps,district,cell,bgroup,area, validity_start,validity_end, pre_test_date, post_test_date, last_updated,lastupdateby,first_apply_by, first_apply_dt, visa, visaexp, corona, temp, passtype)" &
                ...
                ", '" & txtNewVillage.Text.Replace("'", "") & "','" & txtNewPO.Text.Replace("'", "") & "','" & txtNewPS.Text.Replace("'", "") & "','" & txtNewDistrict.Text.Replace("'", "") & "','" & txtNewCell.Text.Replace("'", "") & "','" & ddlBlood.SelectedValue & "','" & ddlNewWorkArea.SelectedValue & "','" & validity_start.ToString("yyyy-MM-dd") & "','" & validity_end.ToString("yyyy-MM-dd") & "','" & pre_test_date.ToString("yyyy-MM-dd") & "','" & post_test_date.ToString("yyyy-MM-dd") & "',current_timestamp,'" & Session("email") & "','" & Session("email") & "',current_timestamp,'" & txtNewVisa.Text.Replace("'", "") & "','" & visaexp & "','" & rblCorona.SelectedValue & "', '" & temperature.ToString & "','" & rblNewPassType.SelectedValue & "' )"
        ...
        ElseIf lbNewMode.Text = "Edit" Then
            ...
            Dim q = "update gatepass set name = '" & txtNewName.Text.Replace("'", "") & "',father='" & txtNewFather.Text.Replace("'", "") & "',sex='" & ddlNewSex.SelectedValue & "',age='" & age & "',dob='" & dob.ToString("yyyy-MM-dd") & "',nationality='" & ddlNewNation.SelectedValue & "',nid='" & txtNewNID.Text.Replace("'", "") & "',mainagency='" & ddlNewVendor.SelectedValue & "'," &
                ...
                "cell='" & txtNewCell.Text.Replace("'", "") & "',bgroup='" & ddlBlood.SelectedValue & "',area='" & ddlNewWorkArea.SelectedValue & "',compliance = '" & ddlNewCompliance.SelectedValue & "', validity_start='" & validity_start.ToString("yyyy-MM-dd") & "',validity_end='" & validity_end.ToString("yyyy-MM-dd") & "', pre_test_date='" & pre_test_date.ToString("yyyy-MM-dd") & "',post_test_date='" & post_test_date.ToString("yyyy-MM-dd") & "', visa = '" & txtNewVisa.Text.Replace("'", "") & "', visaexp = '" & visaexp & "', corona='" & rblCorona.SelectedValue & "',temp = '" & txtNewTemperature.Text.Replace("'", "") & "',last_updated= current_timestamp,lastupdateby = '" & Session("email") & "'" & qPart1 & qPart2 & " where id =" & lbID.Text
        ...
        ElseIf lbNewMode.Text = "Renewal" Then
            ...
            Dim q = "update gatepass set name = '" & txtNewName.Text.Replace("'", "") & "',father='" & txtNewFather.Text.Replace("'", "") & "',sex='" & ddlNewSex.SelectedValue & "',age='" & age & "',dob='" & dob.ToString("yyyy-MM-dd") & "',nationality='" & ddlNewNation.SelectedValue & "',nid='" & txtNewNID.Text.Replace("'", "") & "',mainagency='" & ddlNewVendor.SelectedValue & "'," &
                ...
                "cell='" & txtNewCell.Text.Replace("'", "") & "',bgroup='" & ddlBlood.SelectedValue & "',area='" & ddlNewWorkArea.SelectedValue & "',compliance = '" & ddlNewCompliance.SelectedValue & "', validity_start='" & validity_start.ToString("yyyy-MM-dd") & "',validity_end='" & validity_end.ToString("yyyy-MM-dd") & "', pre_test_date='" & pre_test_date.ToString("yyyy-MM-dd") & "',post_test_date='" & post_test_date.ToString("yyyy-MM-dd") & "', temp='" & txtNewTemperature.Text.Replace("'", "") & "', visa = '" & txtNewVisa.Text.Replace("'", "") & "', passtype = '" & rblNewPassType.SelectedValue & "', visaexp = '" & visaexp & "', corona='" & rblCorona.SelectedValue & "',last_updated= current_timestamp,lastupdateby = '" & Session("email") & "'" & qPart1 & ", remark =  '> " & lbRemark.Text & "' || ifnull(remark,'') where id =" & lbID.Text
            '' update existing data
            Dim ret = executeDB(q, "gpdb")
            If Not ret.Contains("error") Then
        ...
        ElseIf lbNewMode.Text = "Modify" Then
            ...
            Dim q = "update gatepass set name = '" & txtNewName.Text.Replace("'", "") & "',father='" & txtNewFather.Text.Replace("'", "") & "',sex='" & ddlNewSex.SelectedValue & "',age='" & age & "',dob='" & dob.ToString("yyyy-MM-dd") & "',nationality='" & ddlNewNation.SelectedValue & "',nid='" & txtNewNID.Text.Replace("'", "") & "',mainagency='" & ddlNewVendor.SelectedValue & "'," &
                ...
                " cancelremark = '" & rblNewCancelRemark.SelectedValue & "', cell='" & txtNewCell.Text.Replace("'", "") & "', validity_start='" & validity_start.ToString("yyyy-MM-dd") & "',validity_end='" & validity_end.ToString("yyyy-MM-dd") & "', pre_test_date='" & pre_test_date.ToString("yyyy-MM-dd") & "',post_test_date='" & post_test_date.ToString("yyyy-MM-dd") & "', temp = '" & txtNewTemperature.Text.Replace("'", "") & "', bgroup='" & ddlBlood.SelectedValue & "',area='" & ddlNewWorkArea.SelectedValue & "',compliance = '" & ddlNewCompliance.SelectedValue & "', last_updated= current_timestamp, visa = '" & txtNewVisa.Text.Replace("'", "") & "', visaexp = '" & visaexp & "', corona='" & rblCorona.SelectedValue & "',lastupdateby = '" & Session("email") & "', remark =  '> " & newRemark & "' || ifnull(remark,''), status = '" & ddlNewChangeStatus.SelectedValue & "' " & qPart2 & " where id =" & lbID.Text
            '' update existing data
            Dim ret = executeDB(q, "gpdb")
            If Not ret.Contains("error") Then
    ...
    End Sub
    Private Sub gvModifyTemp_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gvModifyTemp.RowCommand
        Dim value = e.CommandArgument.ToString()
        lbModTempMessage.Text = value
        Response.Redirect("gatepass.aspx?id=" & value & "&mode=Renewal")
    End Sub
    ...
    Private Sub ddlRenewAgency_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlRenewAgency.SelectedIndexChanged
        loadRenewalGrid()
    End Sub
    Private Sub ddlModTempAgency_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlModTempAgency.SelectedIndexChanged
        loadModTempGrid()
    End Sub
...
Private Sub btnDeletePics_Click(sender As Object, e As EventArgs) Handles btnDeletePics.Click
      Dim q = "select id, last_updated, validity_end from gatepass where status = 'Cancelled' order by last_updated asc limit 10000"
      dim msg = ""
       Dim nwDT = getDBTable(q, "gpdb")
                For Each r In nwDT.Rows
                    Dim file_name As String = Server.MapPath("./upload/gatepass/") + r("id").ToString + ".jpg"
                   If File.Exists(file_name) Then
                    System.IO.File.Delete(file_name)
                        msg= msg & " " & r("id").ToString 
                   End If
                Next
          
        lbCreateID.Text = "Deleteing " & msg
